/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/docs/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ManagerWhereUniqueInput } from "../../manager/base/ManagerWhereUniqueInput";
import { ApiProperty } from "@nestjs/swagger";
@InputType()
class ManagerUpdateManyWithoutSuppliersInput {
  @Field(() => [ManagerWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [ManagerWhereUniqueInput],
  })
  connect?: Array<ManagerWhereUniqueInput>;

  @Field(() => [ManagerWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [ManagerWhereUniqueInput],
  })
  disconnect?: Array<ManagerWhereUniqueInput>;

  @Field(() => [ManagerWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [ManagerWhereUniqueInput],
  })
  set?: Array<ManagerWhereUniqueInput>;
}
export { ManagerUpdateManyWithoutSuppliersInput };
